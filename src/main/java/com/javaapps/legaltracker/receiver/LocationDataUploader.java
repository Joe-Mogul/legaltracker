...
public class LocationDataUploader extends BroadcastReceiver{
	...
	private UploadHandler uploadHandler=new LocationDataUploaderHandler();
	...
	public void onReceive(Context context, Intent intent) {
	    ...
	    {
	    	Log.i("legaltrackeruploader","cannot upload location data because wifi is not enabled");
	    	return;
	    }
	    if (!wifiManager.pingSupplicant()){
	    	Log.i("legaltrackeruploader","cannot upload location data because could not could to wifi");
	    	return;
		...
		{
		legalTrackerFile.readFromObjectFile(uploadHandler);
		}
